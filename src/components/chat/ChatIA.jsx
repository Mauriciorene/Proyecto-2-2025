...
import { Button, Form, ListGroup, Spinner, Modal } from "react-bootstrap";
    ...
    // Función real para obtener respuesta de IA usando Google Gemini
    const obtenerRespuestaIA = async (promptUsuario) => {
    const apiKey = import.meta.env.VITE_GOOGLE_AI_API_KEY;

    const prompt = `Extrae el nombre y la descripción de categoría en este mensaje: "${promptUsuario}". Si el usuario no provee una descripción, genera una descripción corta basándote en el nombre. Asegúrate que el nombre y descripción comiencen con mayúsculas. Devuélvelo en JSON como {"nombre": "...", "descripcion": "..."}.`;
    ...
    const enviarMensaje = async () => {
        ...
        try {
            ...
            await addDoc(chatCollection, {
                texto:
                "No se pudo registrar la categoría. El JSON no contiene la información esperada.",
                emisor: "ia",
                timestamp: new Date(),
            });
            }
